...
library SafeHRC20 {
    ...
    function safeTransfer(IHRC20 token, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(IHRC20 token, address from, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
    }

    function safeApprove(IHRC20 token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(IHRC20 token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).add(value);
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(IHRC20 token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).sub(value, "SafeHRC20: decreased allowance below zero");
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accOytPerShare;
        uint16 depositFeeBP;
    }


    OgeeYield public oyt;
    address public devaddr;
    uint256 public oytPerBlock;
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _oytPerBlock,
        uint256 _startBlock
    ) public {
        oyt = _oyt;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        oytPerBlock = _oytPerBlock;
    ...
    function add(uint256 _allocPoint, IHRC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner {
    ...
    function pendingOyt(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accOytPerShare = pool.accOytPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 oytReward = multiplier.mul(oytPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accOytPerShare = accOytPerShare.add(oytReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accOytPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accOytPerShare = pool.accOytPerShare.add(oytReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accOytPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safeOytTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accOytPerShare).div(1e12);
    ...
    function safeOytTransfer(address _to, uint256 _amount) internal {
        uint256 oytBal = oyt.balanceOf(address(this));
        if (_amount > oytBal) {
            oyt.transfer(_to, oytBal);
        } else {
            oyt.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _oytPerBlock) public onlyOwner {
        massUpdatePools();
        oytPerBlock = _oytPerBlock;
    }
}
...
contract OgeeYield is HRC20('Ogee Yield Token', 'OYT') {
        ...
        external
    {
        ...
        require(signatory != address(0), "OYT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "OYT::delegateBySig: invalid nonce");
        require(now <= expiry, "OYT::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "OYT::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying OYTs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "OYT::_writeCheckpoint: block number exceeds 32 bits");
